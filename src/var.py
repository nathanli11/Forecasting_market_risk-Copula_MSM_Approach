"""VaR forecasting helpers for copula-MSM replication
...
from src.msm import msm_mixture_cdf, msm_mixture_quantile
...
def portfolio_returns(returns: pd.DataFrame, weights: Iterable[float] = (0.5, 0.5)) -> pd.Series:
    """Compute the two-asset portfolio return series used for VaR backtests."""
...
def violation_frequency(realized_portfolio_returns: pd.Series, var_forecasts: pd.Series) -> float:
    """Return the empirical frequency of VaR violations."""
...
def var_loss_series(r: pd.Series, var: pd.Series, alpha: float) -> pd.Series:
    """
...
def smooth_var_loss_series(r: pd.Series, var: pd.Series, alpha: float, nu: float = 25.0) -> pd.Series:
    """
...
def build_loss_panel(portfolio_returns: pd.Series, var_panel: pd.DataFrame, alpha: float, smooth: bool = False, nu: float = 25.0) -> pd.DataFrame:
    losses = {}
